...
<head>
    ...
    <title>Online Voting - Set Candidate Registration Date</title>
    ...
    <script type="text/javascript">
        ...
        // Retained and improved form validation
...
<div class="container">
    ...
    <div class="navbar">
        <ul>
            ...
            <li class="active"><a href="regcan_date.php">r_candidate date</a></li>
            <?php
            // Fetch voter registration date
            $resultDate = $conn->query("SELECT * FROM voter_reg_date LIMIT 1");
            $dateRes = $resultDate->fetch_assoc();
            $startDate_voter = $dateRes['start'] ?? null;
            $endDate_voter = $dateRes['end'] ?? null;
            $current = date("Y-m-d");

            if ($startDate_voter && $endDate_voter && $current >= $startDate_voter && $current <= $endDate_voter) {
    ...
    <div class="content-area">
        ...
        <div class="main-content">
            <div class="desk">
                ...
                <h2><u>Specify Candidate Registration Dates:</u></h2>
                
                <form name="myform" method="post" class="date-form" onsubmit="return validateForm()">
                    <table>
                        <tr>
                            <td>Start Date</td>
                ...
                if (isset($_POST['setDate'])) {
                    ...
                    // Validation is handled client-side via JS, but double-check server-side for security
                    if (empty($stdate) || empty($endate)) {
                        ...
                        // Delete existing dates (assuming only one range is active)
                        if (!$conn->query("DELETE FROM candidate_reg_date")) {
                            echo "<p style='color:red;'>Error clearing old date: " . $conn->error . "</p>";
                        } else {
                            // Insert new dates using prepared statement
                            $stmt = $conn->prepare("INSERT INTO candidate_reg_date (start, end) VALUES (?, ?)");
                            $stmt->bind_param("ss", $stdate, $endate);
                            
                            if ($stmt->execute()) {
                                echo "<p style='color:green;'>**Candidate registration dates set successfully!**</p>";
